use Test::Stream -V1, -SpecTester, Compare => '*', Class => ['Test::Stream::Compare::Set'];

tests construction => sub {
    my $one = $CLASS->new();
    isa_ok($one, 'Test::Stream::Compare', $CLASS);
...
tests add_check => sub {
    ...
    is(
        $one->checks,
        [ 'a', 'b', meta { prop blessed => 'Test::Stream::Compare::Pattern' } ],
        "Added the checks"
    );
};
