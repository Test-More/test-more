use Test::Stream -V1, Mock, Compare => '*', Class => ['Test::Stream::Workflow'];
...
{
    package Bar;
    use Test::Stream::Exporter qw/default_export/;
    use Test::Stream::Workflow '-all';
    ...
    default_export foo => sub { 'foo' };

    package Baz;
    Bar->import();
    Bar->unimport;
    use Test::Stream 'Core', 'Compare', 'Mock';
    use Test::Stream::Workflow '-all';
    imported_ok(qw/foo/);
    is(foo(), 'foo', "got foo");
    ...
    my $args;
    my $mock = mock 'Test::Stream::Workflow::Meta' => (
...
ok(Test::Stream::Workflow::Meta->get('Baz'), "Baz got a workflow");
ok(!Test::Stream::Workflow::Meta->get('Baz')->autorun, "unimported (autoload set to false)");
...
my $meta = Test::Stream::Workflow::Meta->build(__PACKAGE__, __FILE__, __LINE__, __LINE__);
$meta->set_autorun(0);
...
my $line = __LINE__ + 2;
is(
    dies { sub { Test::Stream::Workflow::die_at_caller([caller()], 'foo bar') }->() },
...
like($current->{add_primary}, object { prop blessed => 'Test::Stream::Workflow::Unit' }, "added primary");
...
like(
    ...
    array {
        ...
        item object {
            prop blessed => 'Test::Stream::Workflow::Unit';
            call type => 'single';
            call wrap => F();
        };
        item 'primary';
