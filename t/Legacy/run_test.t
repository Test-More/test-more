...

{
    ...
    my ($prem, @results) = run_tests(sub { $cap->ok(1, "run pass") });
...
{
    my ($prem, @results) = run_tests(sub { $cap->ok(0, "run fail") });
...
{
    my ($prem, @results) = run_tests(sub { $cap->skip("just because") });
...
{
    my ($prem, @results) = run_tests(sub { $cap->todo_skip("just because") });
...
{
    my ($prem, @results) = run_tests(sub { $cap->diag("run diag") });
...

{
    ...
    $test->is_eq(
        $res_pass->{diag}, "multi pass diag1\nmulti pass diag2\n",
        "run multi pass diag"
    );
