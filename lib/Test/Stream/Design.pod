...
Test::Builder is one of the most important namespaces on cpan. Breaking
Test::Builder would effectively break cpan. There needs to be a plan in place,
acceptible to the community, that limits the chances of breaking everything.

=head2 Specification

Don't break things. Do not anger the perl-qa or toolchain groups. Do not create
a reason for a system that lets people get namespaces taken away. Do not get
tar and feathered.

The decision for release plan is going to be a topic at the Perl-QA meeting in
Berlin which will happen in April of 2015. No plan will be picked prior to that
discussion.

=head2 Implementation Proposals

=head3 Fully Integrated

This is the more contraversial option. This would be a release of a new
non-trial Test-Simple dist where Test::Builder, Test::More, and Test::Simple
are all updated to use Test::Stream. This one scares people, and fear is
perfectly justified.

For this to work we need to continue the blogpost campaign, frequent dev
releases, and lots of perl-blead smoking.

L<https://github.com/Test-More/test-more/tree/stream/replace> is the branch
that implements this proposal.

=head3 Untouched Legacy, bundled stream

This is the middle ground as far as contraversy goes. Test/Builder.pm,
Test/Simple.pm, Test/More.pm and other pre-existing modules in the dist will be
completely unchanged in teh new release. Any existing tool will continue to
work as expected using the legacy code.

Test::Stream will also be part of the dist, but will not be loaded
automatrically by any existing tool. Loading Test::Stream will result in
alternative Test::Builder, Test::More, and Test::Simple implementations. The
only way anyone will be 'forced' to use Test::Stream is if they use a tool that
loads it.

The contraversy here is that Test::Stream being integrated into the dist is an
endorsement. Being integrated also means it will quickly become core with the
rest of Test-Simple. Some members of the perl-qa group are reluctant to accept
Test::Stream due to lack of review, and lack of involvement in the early design
phase.

L<https://github.com/Test-More/test-more/tree/stream/coexist> is the branch
that implements this proposal.

=head3 Seperate dists for a trial period

The Test-Simple dist will not be modified in any way, except to document that
the Test::More, Test::Simple, and Test::Builder namespaces are effectively
locked to all non-critical changes. It will be made known that there are
efforts to write a new base for testing tools. All efforts will recieve equal
documentation in the Test::Builder pod.

Test::Stream will be released as its own independant dist. Apart from being a
seperate dist there is no effective difference from the previous proposal.
Loading Test::Stream will load alternative Test::Builder implementations. Any
tool that does not use Test::Stream will use the original Test-Builder code.

Eventually a new, widely accepted base for testing tools will be picked, and
integrated into the Test-Simple dist. Test::Stream may be this implementation,
it may not be. I will focus my efforts on making Test::Stream into an
acceptable implementation, I will not be joining any other efforts, or starting
from scratch, unless I can be convinced it is really necessary.

If someone else decides to write an alternative testing base, I will not
protest. If a new testing base library does become widely accepted I will not
block its inclusion in Test-Simple unless it badly breaks backwords
compatability.

L<https://github.com/Test-More/test-more/tree/stream/coexist> is the branch
that implements this proposal and the previous (some files will need to be
removed for the Test-Simple dist).
...
methods, specially when implemented poorlyi, are significantly slower than
