...
without any extra arguments, plugins, or bundles.
...
to check that 2 reference are the same exact reference, though you can check
that they have the exact same data. If you actually want to check if 2 refs are
the same or different you can use C<same_ref()> or C<diff_ref()>.
...
clear to you it is there. For really long strings (long enough to wrap) it will
also give you a character count to help with your limited human ability to spot
differences.
...
comparisons:
    ...
    like(
        { a => 'aaa', b => 'bbb', c => ['a' .. 'z'], d => 'ddd' },
        {
            ...
            b => sub { $_[0] eq 'bbb' }, # Code is run to check result
...
Not loaded by default, you must ask for it:
    ...
    use Test::Stream qw/-Default Subtest/;    
...
The old output format can be used if requested:
    ...
    use Test::Stream '-Default', 'Subtest' => ['streamed'];
...
C<isa>. All caps is used to reflect the UNIVERSAL property used, it also helsp
...
Returns an arrayref of all the warnings from within the codelbock, undef if
...
Loading L<Test::Stream> with no arguments (Default bundle) will turn on
strictures for you.
...
Forking and threading in your tests will just work.
