...
This is what Test::Stream is named Test::Stream, all events hub from the
...
previously required to accomplish this. Any downhub tool can also link 'ok'
...
your module generates. This is helpful in ensuring changes uphub do not
...
=item Test::Stream::HashBase::Meta
...
test passes downhub. This is one of few core modules I am willing to do this
...
unbelievable amount of downhub things appear to depend on them.

=back

=head2 Test::Stream

The state object has an extra field called 'legacy'. This is an array of all
events of some types. Test::Builder used to maintain an array of hashes
representing events for inspection later. Code which relied on this capability
now depends on this and some translation logic in Test::Builder.
...
number of downhub tools, so we have to stay forgiving for now.
...
This makes use of the C<< state->legacy >> data mentioned in the Test::Stream
section.  This also needs to do some gymnastics and monkeypatching for $Level
support.
...
=item This module is a hub through which events hub
...
Test::Stream::HashBase and the attribute constants. This exporter is a very
