...
have a context object you can ask it to issue events for you.
...
namespace. C<< $context->send_event( 'Type', ... ) >> will automatically load
C<Test::Stream::Event::Type> if it exists.
...
=item All event types

    $ctx->send_Event( 'Type', ... );

This will send an event of C<Test::Stream::Event::Type>, loading the module if
necessary. The arguments are attributes of the event type as key/value pairs.
...
not allow you to test all events, and they are very fragile when uphub libs
change.
