...
Typically, authors name their tests with a number, and a short name signifying
what the test tests. For example, F<00-load.t> is a common test name, and should
test that the module can be loaded. F<01-methodname.t> might then test that a
particular function does what it should. By incrementing the number at the
beginning of the filename, you can maybe get the tests to run in a particular
order; see L<above|/"How do I get tests to run in a certain order?">.

Another common naming scheme uses bug numbers, names, or mnemonics for
regression tests: F<rt-54321.t>, or F<rt54321-silly-mistake.t> might test that
the fix for a particular bug is there and working right.
...
Install Test::More into t/lib under your source directory.  Then in your tests
say C<use lib 't/lib'>.
...
Technically you don't even need C<ok()>, but that doesn't mean it's a good idea
to go without. Beyond C<ok()>, you can get multiple ways to say "ok" clearly,
you can getdeep comparisons of data structures, you can test future features,
and you can skip tests. It really is worth it.
