...
the manual says it does. [3] Let's pull something out of of 
...
Test::More provides more informative ways to say 'ok'.  C<ok()> is nice
and generic, you can write almost any test with it, but it can't tell
you what went wrong.  For that, we use the C<is()> function.
