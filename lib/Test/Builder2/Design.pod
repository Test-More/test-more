...
writing a test event watcher.  Everything of interest in a test, from setting 
the plan to diagnostic output to asserts, triggers an event. A watcher can take 
action based on an event, or even alter an event as it happens.  Event watchers
will allow TB2 to halt on test failure, to enforce test names, and to provide the 
functionality of Test::NoWarnings.  

Formatters are also event watchers.  See L<Test::Builder2::EventWatcher>, 
