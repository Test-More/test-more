...
=item Record when a test starts and ends
...
itself of its responsibility to B<all> Perl users everywhere.
...
Perl programmers.  It knocks the entire modern suite of Perl testing tools
from their hands.
...
I<de facto> eliminates that environment from serious use by Perl.  It can make
...
The third is by hooking into events, such as when an assert starts or
finishes.
...
can easily find the failing assert.  This is one of the trickiest aspects of
TB1 and TB2.  It has its own section, L</File and Line Information>.
...
TB2's event system, rather than having explicit event callbacks, is modelled
