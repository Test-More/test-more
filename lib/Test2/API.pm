...
sub event_stream {
    my ($stream_name, $handle, %params) = @_;
    ...
    binmode($handle, ':via(PerlIO::via::Test2)') or die "Could not add Test2 PerlIO Layer: $!";
...
=head2 test2_stdout() and test2_stderr()

Usage:

    my $stdout = test2_stdout();
    my $stderr = test2_stderr();

    print $stdout "This goes to STDOUT\n";
    print $stderr "This goes to STDOUT\n";

It is always possible that code you are testing could use C<select> or replace
STDOUT and STDERR. Test2 duplicates STDOUT and STDERR during initialization to
ensure you always have access to the real ones. L<Test2::Formatter> classes
should use these handles instead of printing to STDOUT or STDERR directly.

You should B<NEVER> use C<event_stream()> on these, or do anything else "funny"
to these filehandles. The only exception is that it is safe (and normal) for
some formatters to turn on autoflush for these handles, though Test2::API does
...
=head2 event_stream($NAME => $HANDLE, %PARAMS)

Usage:

    event_stream('STDOUT' => \*STDOUT);
    event_stream('STDERR' => \*STDERR, diagnostics => 1);

This will make sure anything printed to STDERR or STDOUT (including exceptions
and warnings) will be turned into L<Test2::Event::Output> events.
...
The name of the handle, B<THIS MUST BE UNIQUE>.

=item $HANDLE

A reference to an IO handle. The L<PerlIO::via::Test2> PerlIO layer will be
applied to the handle.

=item %PARAMS

Any additional parameters you wish to use for L<PerlIO::via::Test2>
construction. At the moment 'diagnostics' is the only useful parameter.

=over 4

=item diagnostics => $bool

This is passed in to all L<Test2::Event::Output> events generated by this
stream. Some formatters treat diagnostics differently than non-diagnostics, or
print them to different output handles. For example, STDOUT should have
diagnostics set to 0, STDERR should have it set to 1.
...
=item Cannot be used with some formatters

If the formatter you want to use prints directly to the global STDERR or
STDOUT, and you apply this layer to them, you will find yourself in an infinite
non-recursive loop.

=item Cannot intercept output that bypasses perlio

This will not apply to duplicates of the filehandles. This will also not apply
to output generated by a forked process that inherits STDOUT and STDERR. For
example C<system("echo hi")> will not be intercepted, it will not become an
event.
