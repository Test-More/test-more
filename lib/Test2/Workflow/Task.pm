...
use overload(
    ...
    '&{}' => sub {
        my $self = shift;
        my @caller = caller(0);
        update_mask($caller[1], $caller[2], '*', {restart => 1, pause => 1, 3 => 'CONTINUE'});
...
use Test2::Workflow::Util qw/CAN_SET_SUB_NAME set_sub_name update_mask/;
...
sub _really_run_primary {
    ...
    if(reftype($primary) eq 'ARRAY') {
        ...
        BEGIN { update_mask(__FILE__, __LINE__ + 1, '*', {pause => 1, restart => 1, shift => 1}) }
        $primary->(@{$self->{+ARGS}});
    }
}
...
sub _run_primaries {
    ...
    for my $mod (@$modifiers) {
        ...
        my $temp = Test2::Workflow::Unit->new(
            %$mod,
            primary => $primary,
        );
