...
down at the core to just storing, formatting and testing results.  It
...
TB1, only doing TAP, is fortunate in that it's a test.  It can print a
...
Takes the formatted test and outputs it, usually to STDOUT and
...
An ambiguous term often used to mean an assert, a test or a suite.
